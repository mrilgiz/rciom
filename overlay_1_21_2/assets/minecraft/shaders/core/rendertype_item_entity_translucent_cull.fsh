...
float eyeVeins(vec2 center) {
    ...
    // Простые статичные вены без циклов
    float veins = max(abs(center.x * 8.0), abs(center.y * 8.0));
    veins = 1.0 - smoothstep(0.0, 0.2, fract(veins));
    
    return veins * scleraMask * 0.3;
}

vec4 eyeBase(vec2 uv) {
    vec2 center = floor(uv * 8.0) / 8.0 - 0.5; // Еще меньше разрешение
    ...
    // Убираем моргание и движение - статичный глаз
    vec3 eyeColor = vec3(0.9, 0, 0);
    eyeColor = mix(eyeColor, vec3(1, 0.9, 0.9), iris);
    eyeColor = mix(eyeColor, vec3(0.7, 0, 0), pupil);
    
    return vec4(eyeColor, sclera);
}
...
vec4 fearEffect(vec2 uv) {
    // Минимальный статичный эффект страха
    vec2 center = uv - 0.5;
    float dist = length(center);
    
    float fearIntensity = 1.0 - smoothstep(0.0, 0.5, dist);
    vec3 fearColor = vec3(0.1, 0.0, 0.2);
    
    return vec4(fearColor * fearIntensity * 0.2, fearIntensity * 0.1);
}
...
vec4 eyeEffect(vec2 uv) {
    ...
    vec3 color = eye.rgb + veins.rgb * 0.3 + fear.rgb;
    return vec4(color, eye.a);
}
