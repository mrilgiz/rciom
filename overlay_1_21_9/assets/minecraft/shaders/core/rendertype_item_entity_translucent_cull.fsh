...
#moj_import <minecraft:finish_phantom.glsl>
...
vec4 finishCrate(vec4 rgb, vec2 dim, vec2 uv) {
    ...
    float x = crateTile(uv, GameTime) * 0.2 + crateTile(rotate(uv, 0.1), GameTime * (2./3.)) * .6;
...
float eyeVeins(vec2 center) {
    vec2 pos = center * 8.0;
    float veins = 0.0;
    
    // Упрощенные вены - только 2 уровня, меньше итераций
    for(int i = 0; i < 4; i++) {
        float angle = float(i) * 1.57; // 90 градусов
        vec2 veinDir = vec2(cos(angle), sin(angle));
        
        float t = dot(pos, veinDir);
        float distToVein = abs(dot(pos, vec2(-veinDir.y, veinDir.x)));
        
        float veinStrength = 1.0 - smoothstep(0.0, 0.15, distToVein);
        veinStrength *= 1.0 - smoothstep(0.0, 2.0, abs(t));
        
        veins = max(veins, veinStrength * 0.6);
    }
    
    float eyeRadius = max(abs(center.x), abs(center.y));
    float scleraMask = step(0.15, eyeRadius) * (1.0 - step(0.4, eyeRadius));
    
    return veins * scleraMask;
}

vec4 eyeBase(vec2 uv) {
    vec2 pixelUV = floor(uv * 16.0) / 16.0; // Уменьшаем разрешение
    vec2 center = pixelUV - 0.5;
    float maxDist = max(abs(center.x), abs(center.y));
    
    float iris = step(0.15, maxDist) * (1.0 - step(0.35, maxDist));
    float pupil = 1.0 - step(0.08, maxDist);
    float sclera = 1.0 - step(0.4, maxDist);
    
    // Упрощенное моргание
    float blinkCycle = fract(GameTime * 50.0);
    float eyelidClose = blinkCycle < 0.05 ? sin(blinkCycle * 20.0 * PI) * 0.3 : 0.0;
    float eyelidMask = step(abs(center.y), 0.5 - eyelidClose);
    
    // Упрощенное движение глаз
    vec2 eyeMove = vec2(sin(GameTime * 100.0) * 0.05, cos(GameTime * 80.0) * 0.03);
    vec2 pupilCenter = center - eyeMove;
    float movingPupil = 1.0 - step(0.08, max(abs(pupilCenter.x), abs(pupilCenter.y)));
    float redSquare = 1.0 - step(0.05, max(abs(pupilCenter.x), abs(pupilCenter.y)));
    
    vec3 eyeColor = vec3(0.9, 0, 0);
    eyeColor = mix(eyeColor, vec3(1, 0.9, 0.9), iris);
    eyeColor = mix(eyeColor, vec3(0.7, 0, 0), movingPupil);
    eyeColor = mix(eyeColor, vec3(0), redSquare);
    
    return vec4(eyeColor * eyelidMask, sclera * eyelidMask);
}

vec4 eyeVeinsEffect(vec2 uv) {
    vec2 pixelUV = floor(uv * 16.0) / 16.0; // Уменьшаем разрешение
    vec2 center = pixelUV - 0.5;
    
    float veins = eyeVeins(center);
    return vec4(vec3(0.8, 0.2, 0.2) * veins, veins);
}

vec4 fearEffect(vec2 uv) {
    vec2 center = uv - 0.5;
    float time = GameTime * 2000; // Замедляем
    
    // Только 2 тени вместо 4
    vec4 fear = vec4(0.0);
    for(int i = 0; i < 2; i++) {
        ...
        float shadowDist = length(center - shadowPos);
        float shadowIntensity = 1.0 - smoothstep(0.0, 0.15, shadowDist);
        
        vec3 shadowColor = vec3(0.2, 0.0, 0.3);
        float pulse = sin(time * 3.0 + offset) * 0.3 + 0.7;
        
        fear.rgb += shadowColor * shadowIntensity * pulse * 0.3;
        fear.a = max(fear.a, shadowIntensity * 0.5);
    }
    
    // Упрощенное глобальное затемнение
    float globalFear = sin(time) * 0.05 + 0.08;
    fear.rgb += vec3(0.05, 0.0, 0.1) * globalFear;
    fear.a = max(fear.a, globalFear * 0.2);
    
    return fear;
}

vec4 singleEye(vec2 uv, vec2 eyePos, float eyeSize, float timeOffset) {
    vec2 localUV = (uv - eyePos) / eyeSize + 0.5;
    ...
    // Части глаза
    float sclera = 1.0 - step(0.4, dist);
    float iris = step(0.15, dist) * (1.0 - step(0.3, dist));
    float pupil = 1.0 - step(0.1, dist);
    
    // Моргание
    float blinkTime = GameTime * 80.0 + timeOffset;
    float blink = fract(blinkTime);
    float eyelidClose = blink < 0.1 ? sin(blink * 10.0 * PI) * 0.6 : 0.0;
    float eyelidMask = step(abs(center.y), 0.5 - eyelidClose);
    
    // Движение зрачка
    vec2 lookDir = vec2(sin(GameTime * 150.0 + timeOffset), cos(GameTime * 120.0 + timeOffset * 1.3)) * 0.08;
    vec2 pupilCenter = center - lookDir;
    float movingPupil = 1.0 - step(0.1, max(abs(pupilCenter.x), abs(pupilCenter.y)));
    
    // Цвета
    vec3 eyeColor = vec3(0.95, 0.9, 0.9); // Белок
    eyeColor = mix(eyeColor, vec3(0.8, 0.2, 0.2), iris); // Красная радужка
    eyeColor = mix(eyeColor, vec3(0.1, 0.0, 0.0), movingPupil); // Черный зрачок
    
    return vec4(eyeColor * eyelidMask, sclera * eyelidMask);
}

vec4 multipleEyes(vec2 uv) {
    vec4 result = vec4(0.0);
    vec4 energy = vec4(0.0);
    
    // Массив позиций и размеров глаз
    vec2 eyePositions[12];
    float eyeSizes[12];
    float timeOffsets[12];
    
    // Инициализация позиций глаз
    eyePositions[0] = vec2(0.2, 0.3);
    eyePositions[1] = vec2(0.7, 0.2);
    eyePositions[2] = vec2(0.1, 0.7);
    eyePositions[3] = vec2(0.8, 0.6);
    eyePositions[4] = vec2(0.4, 0.1);
    eyePositions[5] = vec2(0.6, 0.8);
    eyePositions[6] = vec2(0.9, 0.4);
    eyePositions[7] = vec2(0.3, 0.9);
    eyePositions[8] = vec2(0.5, 0.5);
    eyePositions[9] = vec2(0.15, 0.5);
    eyePositions[10] = vec2(0.85, 0.15);
    eyePositions[11] = vec2(0.65, 0.35);
    ...
    // Рендерим все глаза и их энергию
    for(int i = 0; i < 12; i++) {
        vec4 eye = singleEye(uv, eyePositions[i], eyeSizes[i], timeOffsets[i]);
        result = mix(result, eye, eye.a);
        
        // Энергетическое свечение от каждого глаза
        float distToEye = length(uv - eyePositions[i]);
        float energyRadius = eyeSizes[i] * 2.5;
        float energyIntensity = 1.0 - smoothstep(0.0, energyRadius, distToEye);
        
        // Пульсация энергии
        float pulse = sin(GameTime * 200.0 + timeOffsets[i] * 10.0) * 0.3 + 0.7;
        energyIntensity *= pulse;
        
        // Цвет энергии - красно-фиолетовый
        vec3 energyColor = mix(
            vec3(0.8, 0.1, 0.2),
            vec3(0.4, 0.0, 0.6),
            sin(GameTime * 300.0 + timeOffsets[i] * 5.0) * 0.5 + 0.5
        );
        
        energy.rgb += energyColor * energyIntensity * 0.3;
        energy.a = max(energy.a, energyIntensity * 0.4);
    }
    
    // Смешиваем глаза с энергией на прозрачном фоне
    result.rgb += energy.rgb;
    result.a = max(result.a, energy.a);
    
    return result;
}

vec4 eyeEffect(vec2 uv) {
    vec4 eye = eyeBase(uv);
    vec4 veins = eyeVeinsEffect(uv);
    vec4 fear = fearEffect(uv);
    
    // Упрощенное смешивание
    vec3 finalColor = mix(eye.rgb, veins.rgb, veins.a * 0.8);
    finalColor = mix(finalColor, fear.rgb, fear.a * 0.4);
    
    return vec4(finalColor, max(eye.a, fear.a * 0.2));
}

vec4 screenEffect(vec2 uv) {
    vec2 center = uv - 0.5;
    
    // Сканлайны
    float scanlines = sin(uv.y * 800.0) * 0.04 + 0.96;
    ...
    // Искривление экрана
    vec2 curved = center;
    curved *= 1.0 + dot(curved, curved) * 0.2;
    curved += 0.5;
    ...
    // Мерцание
    float flicker = sin(GameTime * 60.0) * 0.02 + 0.98;
    
    // Помехи
    float noise = fract(sin(dot(uv, vec2(12.9898, 78.233)) + GameTime * 10.0) * 43758.5453);
    noise = noise * 0.1 + 0.9;
    
    // Цвет экрана
    vec3 color = vec3(
        0.9 + sin(curved.x * 10.0 + GameTime * 5.0) * 0.1,
        0.95,
        0.8 + cos(curved.y * 8.0 + GameTime * 3.0) * 0.1
    );
    
    color *= scanlines * vignette * flicker * noise;
    color = mix(color, vec3(0.2, 1.0, 0.3), 0.1);
...
vec4 heartbeatEffect(vec2 uv) {
    ...
    // Прямоугольное виньетирование в стиле Minecraft
...
void main() {
    ...
    if (finish > 0.) {
        ...
        noshade = 1.;
        e = 1.;
    } else if (floor(finish) == -5.) {
        rgb = eyeEffect(texCoord0);
        noshade = 1.;
        e = 1.;
    } else if (floor(finish) == -6.) {
        vec4 eyesEffect = multipleEyes(texCoord0);
        rgb = eyesEffect;
        noshade = 1.;
        e = 1.;
    }

    // Оптимизированный глобальный эффект страха
    if (finish != -3. && finish != -4. && finish != -5. && finish != -6.) {
        vec4 globalFear = fearEffect(texCoord0);
        rgb.rgb = mix(rgb.rgb, globalFear.rgb, globalFear.a * 0.15);
    }
    ...
    if (floor(finish) == -3.) {
        ...
        fragColor = screenEffect(texCoord0);
        ...
        fragColor = apply_fog(color, sphericalVertexDistance - (1. - e) * 4., cylindricalVertexDistance - (1. - e) * 4., FogEnvironmentalStart, FogEnvironmentalEnd, FogRenderDistanceStart, FogRenderDistanceEnd, FogColor);
    }
}
